...
class GiteeHandler extends AbstractHandler {
    ...
    public function check_update( string $slug, string $version ): ?UpdateInfo {
        ...
        if ( ! empty( $this->source->auth_token ) ) {
            $url = add_query_arg( 'access_token', $this->source->auth_token, $url );
        }
    ...
    public function get_info( string $slug ): ?array {
        ...
        if ( ! empty( $this->source->auth_token ) ) {
            $repo_url = add_query_arg( 'access_token', $this->source->auth_token, $repo_url );
        }
        ...
        if ( ! empty( $this->source->auth_token ) ) {
            $release_url = add_query_arg( 'access_token', $this->source->auth_token, $release_url );
        }
